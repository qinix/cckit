...
type PxnImp struct {
	...
	symbolToMarket     map[string]pxnMarket
}
...
func (b *PxnImp) Monitor() *mm.Exchange {
	return b.monitor
}
...
func (b *PxnImp) subscribeMarket(m pxnMarket) {
...
func (b *PxnImp) writeRequest(r *pxnpb.Request) {
	if b.conn.Subprotocol() == "json" {
...
func (b *PxnImp) getMarkets() (markets []pxnMarket) {
	res, err := http.Get("https://" + b.domain + "/api/v3/asset_pairs")
	if err != nil {
		log.Error(err)
		return
	}
	...
	for _, rawMarket := range response.Data {
		markets = append(markets, pxnMarket{
			ID:    rawMarket.Name,
...
func (b *PxnImp) handleMessage(resp pxnpb.Response) {
...
func (b *PxnImp) mergeDepth(depth *pxnpb.Depth) {
